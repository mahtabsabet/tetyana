...
// ── Questions vocabulaire — PLACEHOLDERS (groupe 3 : remplacez par vos vraies questions !) ──
const QUIZ_QUESTIONS = [
  {
    imageEmoji: '🚀',
    correct: 'La fusée',
    wrong:   ["L'étoile", 'La lune', 'Le soleil'],
  },
  {
    imageEmoji: '⭐',
    correct: "L'étoile",
    wrong:   ['La fusée', 'La planète', 'La lune'],
  },
];
...
export class EspaceScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // this.load.image('espace_bg',   new URL('./assets/backgrounds/bg.png',   import.meta.url).href);
    // this.load.image('espace_coin', new URL('./assets/sprites/collectible.png', import.meta.url).href);
    // this.load.image('espace_goal', new URL('./assets/sprites/goal.png',     import.meta.url).href);
  }
  ...
  create() {
    ...
    this.add.text(W / 2, H / 2 + 100, 'À construire par le groupe 3 !', {
    ...
    // ── Obstacles (météorites — réduisent les points de vie au contact) ───────
    // TODO: Remplacez les emplacements et l'emoji par ceux de votre niveau
    this._obstacles = this.physics.add.staticGroup();
    this._obstacle(this._obstacles, 400,  H - 60, '☄️');
    this._obstacle(this._obstacles, 900,  H - 60, '☄️');
    this._obstacle(this._obstacles, 1400, H - 60, '☄️');
  ...
  _gagner() {
    ...
    this.time.delayedCall(1500, () => {
      showQuiz(this, QUIZ_QUESTIONS, (bonus) => {
        this._score += bonus;
        this._ctx?.onComplete({ levelId:'level03-espace', completed:true, pointsEarned:this._score, starsEarned:3, durationSeconds:0 });
        this._terminer();
      });
    });
  }
